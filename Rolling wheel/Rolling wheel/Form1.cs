...
namespace Rolling_wheel 
{
    ...
    {   
        ...
        {
            if (sender.Equals(button1)) { label1.Text = @"Колесо радиуса R катится по горизонтальной поверхности
...
больше R. такие точки можно найти, например, на реборде -выступе
 железнодорожиого колеса). Требуется построить семейство
траекторий точек колеса."; Height = 230; }
            if (sender.Equals(button2)) { label1.Text = @"
Колесо радиуса R катится по внешней дуге колеса радиуса R1 без 
проскальзыниния. Точка А расположена на расстоянии a от оси колеса.
Требуется построить семейство траекторий точек колеса."; Height = 210; }
            if (sender.Equals(button3)) { label1.Text = @"Колесо радиуса R катится по горизонтальной поверхности
 без проскальзывания, Точка А расположена на расстоянии a
от оси колеса(будем считать, что а может быть как меньше, так и
больше R. такие точки можно найти, например, на реборде -выступе
 железнодорожиого колеса). Требуется построить семейство
